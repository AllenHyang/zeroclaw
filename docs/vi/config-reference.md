...
1. `--config-dir <path>` ghi đè CLI (dùng bởi `workspace start` để khởi chạy daemon mỗi workspace)
2. Biến `ZEROCLAW_WORKSPACE` (nếu được đặt)
3. Marker `~/.zeroclaw/active_workspace.toml` (nếu có)
4. Mặc định `~/.zeroclaw/config.toml`

Bố cục đa workspace:

- Workspace mặc định: `~/.zeroclaw/` (config, brain.db, logs, daemon state)
- Workspace có tên: `~/.zeroclaw-<name>/` (cùng cấu trúc, dữ liệu cách ly)
- Mỗi workspace chạy daemon riêng trên gateway port riêng
- Tạo bằng `zeroclaw workspace clone <name>`; xem [commands-reference.md](commands-reference.md)
...
| `remote` | chưa đặt | Tên workspace từ xa cho ủy quyền xuyên workspace |
...
- Khi `remote` được đặt, tool delegate chuyển tiếp prompt đến daemon của workspace được đặt tên qua HTTP thay vì tạo provider cục bộ. Daemon từ xa dùng config riêng (provider, model, tools). Xác thực dùng peer token được trao đổi khi `workspace clone`.
- Độ sâu ủy quyền từ xa giới hạn tối đa 5 để tránh vòng lặp vô hạn giữa các peer.
- Trên host không phải loopback, nếu workspace đích có `tls_cert` đã cấu hình, `https://` được dùng tự động.
...
# Ủy quyền từ xa: chuyển tiếp đến daemon workspace peer
[agents.agent2]
provider = "anthropic"       # bị bỏ qua khi có remote (peer dùng config riêng)
model = "claude-sonnet-4-6"  # bị bỏ qua khi có remote
remote = "agent2"            # tên workspace từ `zeroclaw workspace list`
```
...
| `paired_tokens` | `[]` | Token bearer đã hash SHA-256 (tự quản lý khi `workspace clone`) |
| `tls_cert` | chưa đặt | Đường dẫn chứng chỉ TLS (PEM); khi đặt cùng `tls_key`, bật HTTPS trên gateway |
| `tls_key` | chưa đặt | Đường dẫn khóa riêng TLS (PEM) |

Lưu ý:

- `paired_tokens` được trao đổi tự động khi `workspace clone` và không nên chỉnh sửa thủ công.
- `tls_cert` / `tls_key` hiện được client delegate dùng để chọn scheme: khi workspace đích có TLS và host không phải loopback, ủy quyền từ xa dùng `https://`.
- Chứng chỉ CA của peer có thể đặt tại `{config_dir}/tls/peers/{workspace_name}-ca.pem` cho tin cậy CA tùy chỉnh khi ủy quyền xuyên workspace.
